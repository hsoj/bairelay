...
impl PendingReply {
	...
	where
		F: FnOnce(&Bc, &crate::bc::xml::BcXml) -> Bc + Send + Sync + 'static,
	{
	...
	where
		F: FnOnce(&Bc, &crate::bc::xml::BcXml) -> Option<Bc> + Send + Sync + 'static,
	{
		let reply: ReplyFn = Box::new(move |bc| {
			let xml = inspect_xml(bc);
			match f(bc, xml) {
...
pub fn inspect_xml(bc: &Bc) -> &crate::bc::xml::BcXml {
	match &bc.body {
...
#[allow(dead_code)]
pub fn reply_200_xml(req: &Bc, xml: crate::bc::xml::BcXml) -> Bc {
	Bc::new_from_xml(
		BcMeta {
