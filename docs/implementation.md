...
Per-camera `debug = true` (`verbose` is the neolink alias) sets `BcCameraOpt::debug`, which unlocks the Baichuan codec's trace-level dumps of decrypted control payloads (`Payload Txt:` / `Extension Txt:` in `crates/core/src/bc/de.rs`) — combine with `RUST_LOG=bairelay_neolink_core::bc::de=trace`. Filters of any width work, up to `-vvv`'s global `trace`. The dumps include credential hashes and camera UIDs; `warn_wire_debug_enabled` reminds the operator at startup. Use for wire-level diagnosis (login rejections on new firmware, unmodelled XML), then turn it back off.
