...
use bairelay::baichuan::bcudp::xml::{C2mQ, IpPort, M2cQr, UdpXml};
use bairelay::wake_server::config::RuntimeConfig;
use bairelay::wake_server::packet::{decode_discovery, encode_discovery};
...
async fn middleman_replies_to_c2m_q_with_register_addr() {
	...
	let server_handle = tokio::spawn(bairelay::wake_server::run_with_sockets(
		cfg,
		bairelay::wake_server::make_registry(),
		middleman,
		register,
		cancel.clone(),
	));
...
use bairelay::baichuan::bcudp::xml::{
...
async fn run_binds_configured_ports_and_cancels_cleanly() {
	...
	let cancel = CancellationToken::new();
	let handle = tokio::spawn(bairelay::wake_server::run(
		cfg,
		bairelay::wake_server::make_registry(),
		cancel.clone(),
	));
...
async fn run_returns_bind_error_on_register_port_collision() {
	...
	let cancel = CancellationToken::new();
	let res = bairelay::wake_server::run(cfg, bairelay::wake_server::make_registry(), cancel).await;
	match res {
		Err(bairelay::wake_server::WakeServerError::Bind { addr, .. }) => {
...
async fn oversized_uid_in_d2m_q_is_rejected() {
	use bairelay::baichuan::bcudp::xml::D2mQ;
...
async fn d2r_r_with_no_anchor_is_dropped() {
	use bairelay::baichuan::bcudp::xml::D2rR;
...
async fn cancel_returns_promptly() {
	...
	let handle = tokio::spawn(bairelay::wake_server::run_with_sockets(
		cfg,
		bairelay::wake_server::make_registry(),
		middleman,
		register,
