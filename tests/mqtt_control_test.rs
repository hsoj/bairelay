...
use bairelay::mqtt::control::{parse_control_message, ControlCommand};
...
fn ir_on_off_auto() {
	...
	assert!(matches!(
		...
		Some(ControlCommand::Ir {
			mode: bairelay::mqtt::control::IrMode::On,
			..
		})
	));
	...
	assert!(matches!(
		off,
		Some(ControlCommand::Ir {
			mode: bairelay::mqtt::control::IrMode::Off,
			..
		})
	));
	...
	assert!(matches!(
		auto,
		Some(ControlCommand::Ir {
			mode: bairelay::mqtt::control::IrMode::Auto,
			..
		})
	));
}
