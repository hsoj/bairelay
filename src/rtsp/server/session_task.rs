...
//! emitted; the SR helpers in `crate::rtsp::server::rtcp` remain for any
...
use crate::rtsp::rtp::RtpCounters;
use crate::rtsp::server::registry::{FirstVideoRtpSlot, SessionRegistry, TrackEntry, TrackKind};
...
async fn dispatch_one(
	...
	let packets = crate::rtsp::server::packetizer::dispatch(frame, &mut track.counters);
...
async fn replay_burst(
	burst: &crate::rtsp::buffer::VideoBurst,
	transport: &Arc<dyn Transport>,
	counters: &mut RtpCounters,
) -> bool {
	use crate::rtsp::codec::{h264, h265, VideoCodec};
...
mod tests {
	...
	use crate::rtsp::provider::{AudioPayload, Frame};
	use crate::rtsp::server::transport::noop_transport_for_tests;
	use bytes::Bytes;
	...
	async fn run_replays_cached_burst_on_startup() {
		...
		last_frame.replace_video(crate::rtsp::buffer::VideoBurst {
			codec: VideoCodec::H264,
	...
	async fn replay_burst_h264_fu_a_path_with_failing_transport() {
		...
		use crate::rtsp::codec::VideoCodec;
		let burst = crate::rtsp::buffer::VideoBurst {
			codec: VideoCodec::H264,
			parameter_sets: vec![vec![0x67, 0x42, 0x00, 0x1f], vec![0x68, 0xce]],
			iframe_nals: vec![{
