...
use crate::rtsp::protocol::auth::UserCred;
use crate::rtsp::provider::StreamProvider;
use crate::rtsp::server::tls::TlsConfig;
					...
					match acceptor.as_ref() {
						None => {
							...
							tokio::spawn(async move {
								crate::rtsp::server::connection::handle_connection(
									stream,
									provider,
									users,
									realm,
...
mod tests {
	use super::*;
	use crate::rtsp::provider::{Frame, StreamError, StreamProvider, SubscriptionHandle};
	...
	impl StreamProvider for NoopProvider {
		async fn subscribe(
			...
			Ok(SubscriptionHandle {
				...
				last_frame: Arc::new(crate::rtsp::buffer::LastFrameBuffer::new()),
				guard: Box::new(()),
			})
		}
	}
