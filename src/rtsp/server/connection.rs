...
use crate::rtsp::protocol::auth::{Nonce, UserCred};
use crate::rtsp::provider::StreamProvider;
use crate::rtsp::server::registry::SessionRegistry;
use crate::rtsp::server::udp_pool::UdpPortPool;
					...
					Ok(_) => {
						...
						loop {
							...
							match try_consume_request(&mut buf) {
								Ok(Some(req_bytes)) => {
									if let Err(e) = crate::rtsp::server::connection::dispatch_request(
										&state, &req_bytes,
									).await {
								...
								Err(e) => {
									...
									let resp = crate::rtsp::protocol::message::build_response(
...
pub(crate) async fn dispatch_request(
	...
	use crate::rtsp::protocol::message::{build_response, parse_request, RtspMethod};
...
async fn handle_getparameter(
	state: &Arc<ConnectionState>,
	cseq: u32,
	parsed: &crate::rtsp::protocol::message::ParsedRequest,
) -> io::Result<()> {
...
async fn handle_describe(
	...
	let Some(resolved) = crate::rtsp::url::resolve(&path) else {
		let resp = crate::rtsp::protocol::message::build_response(
	...
	{
		Ok(s) => s,
		Err(crate::rtsp::provider::StreamError::UnknownCamera) => {
			let resp = crate::rtsp::protocol::message::build_response(
				rtsp_types::StatusCode::NotFound,
				cseq,
				&[],
	...
	let sdp = crate::rtsp::sdp::build(&sdp_params);
...
async fn authenticate(
	...
	match verify_digest(
		...
		Err(AuthError::StaleNonce) => {
			let fresh = crate::rtsp::protocol::auth::Nonce::random();
...
async fn handle_setup(
	...
	use crate::rtsp::protocol::session::new_session_id;
	use crate::rtsp::protocol::transport as transport_mod;
	use crate::rtsp::server::registry::{TrackEntry, TrackKind};
	...
	let Some(resolved) = crate::rtsp::url::resolve(&base_path) else {
		let resp = crate::rtsp::protocol::message::build_response(
			rtsp_types::StatusCode::NotFound,
			cseq,
			&[],
	...
	let (transport_impl, transport_response) =
		match build_transport(state, transport_spec, track_ssrc).await {
			Ok(v) => v,
			Err(status) => {
				let resp = crate::rtsp::protocol::message::build_response(status, cseq, &[], None);
				return write_response(state, &resp).await;
			}
		};
	...
	.await
	{
		...
		Err(crate::rtsp::provider::StreamError::AccessDenied) => {
			let resp = crate::rtsp::protocol::message::build_response(
				rtsp_types::StatusCode::Forbidden,
				cseq,
				&[],
...
async fn build_transport(
	state: &Arc<ConnectionState>,
	transport_spec: crate::rtsp::protocol::transport::TransportSpec,
	track_ssrc: u32,
) -> Result<(Arc<dyn crate::rtsp::server::transport::Transport>, String), rtsp_types::StatusCode> {
	use crate::rtsp::protocol::transport as transport_mod;
	match transport_spec {
		transport_mod::TransportSpec::TcpInterleaved {
			...
			let t = crate::rtsp::server::transport::TcpInterleavedTransport::new(
		...
		transport_mod::TransportSpec::UdpUnicast {
			...
			let t = crate::rtsp::server::transport::UdpUnicastTransport::bind(
				udp_bind_ip,
...
/// surface a 503 to the RTSP client. Other [`crate::rtsp::provider::StreamError`]
...
async fn subscribe_with_extern_fallback(
	state: &Arc<ConnectionState>,
	camera: &str,
	kind: crate::rtsp::url::StreamKind,
	user: Option<&str>,
) -> Result<crate::rtsp::provider::SubscriptionHandle, crate::rtsp::provider::StreamError> {
	match state.provider.subscribe(camera, kind, user).await {
		Err(crate::rtsp::provider::StreamError::Unavailable(_))
			if kind == crate::rtsp::url::StreamKind::Extern =>
		{
			tracing::debug!(camera, "Extern stream unavailable, falling back to Sub");
			state
				.provider
				.subscribe(camera, crate::rtsp::url::StreamKind::Sub, user)
				.await
		}
		other => other,
	}
...
async fn handle_teardown(
	...
	let resp = crate::rtsp::protocol::message::build_response(status, cseq, &extra, None);
	write_response(state, &resp).await
}
...
mod tests {
	...
	use crate::rtsp::protocol::auth::Nonce;
	use crate::rtsp::provider::{Frame, StreamError, SubscriptionHandle};
	...
	impl crate::rtsp::provider::StreamProvider for MockProv {
		async fn subscribe(
			&self,
			camera: &str,
			kind: crate::rtsp::url::StreamKind,
	...
	fn make_state(
		provider: Arc<dyn crate::rtsp::provider::StreamProvider>,
	...
	async fn handle_connection_closes_on_initial_request_timeout() {
		...
		let task = tokio::spawn(async move {
			handle_connection(
				...
				vec![],
				"test".into(),
				Arc::new(crate::rtsp::server::udp_pool::UdpPortPool::new()),
