...
use crate::wake_server::config::RuntimeConfig;
use crate::wake_server::packet::{decode_discovery, encode_discovery, random_sid};
use crate::wake_server::registry::{CameraRegistry, SessionAnchors};
use crate::wake_server::WakeServerError;
...
async fn handle_connect(
	...
	src: SocketAddr,
	tid: u32,
	c: crate::baichuan::bcudp::xml::C2rC,
	cancel: &CancellationToken,
) {
	...
	match cam {
		...
		Some(entry) => {
			...
			let relay_ip_for_camera =
				crate::wake_server::route::advertise_ip(cfg.bind, cam_addr).to_string();
			let relay_ip_for_client =
				crate::wake_server::route::advertise_ip(cfg.bind, src).to_string();
			let relay = IpPort {
...
async fn handle_heartbeat(
	...
	match registry.upsert(&hb.uid, src, hb.token, now) {
		...
		None => {
			warn!(
				%src, uid = %hb.uid,
				cap = crate::wake_server::registry::MAX_MAP_ENTRIES,
