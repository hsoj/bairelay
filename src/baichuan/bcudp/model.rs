...
pub struct UdpAck {
	...
	/// Might be a latency for the received acknoledge packets
	pub maybe_latency: u32,
	// 2 Bytes size of a payload
	//
	/// Payload of `00 01 01 01 01` where `01` is added after every repeat
	///
	/// This is a truth table of packets after `packet_id` that have not been received
	pub payload: Vec<u8>,
}

impl UdpAck {
	/// Create an empty ack packet to signifiy no packets received yet
...
/// Contains the data of a [`crate::baichuan::bc::model::Bc`] packet
...
mod model_unit_tests {
	...
	#[test]
	fn udp_data_debug_redacts_payload() {
		let d = UdpData {
			connection_id: 7,
			packet_id: 99,
		...
		assert!(s.contains("packet_id"));
		// Payload length is surfaced but the bytes themselves are not.
		assert!(s.contains("payload_len"));
		assert!(s.contains('6'), "payload_len=6 should appear");
		assert!(!s.contains("de"), "raw payload bytes should not appear");
	}
}
