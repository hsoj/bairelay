	...
	fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>> {
		...
		match BcUdp::deserialize(src) {
			Ok(BcUdp::Discovery(UdpDiscovery {
				...
				tracing::trace!("   Decoding: Relay terminate");
			...
			Ok(BcUdp::Discovery(UdpDiscovery {
				...
				tracing::trace!("   Decoding:Camera terminate");
				Err(Error::CameraTerminate)
			}
			Ok(bc) => {
				tracing::trace!("   Decoding: Ok");
				Ok(Some(bc))
			}
			Err(Error::NomIncomplete(_)) => {
				tracing::trace!("   Decoding: Incomplete: {:0X?}", src);
				Ok(None)
			}
			Err(e) => {
				tracing::trace!("   Decoding: Err");
				Err(e)
			}
		}
	}
...
mod tests {
	use super::*;
	use crate::baichuan::bcudp::model::{UdpAck, UdpData, UdpDiscovery};
	use crate::baichuan::bcudp::xml::{C2dDisc, D2cDisc, PortList, R2cDisc, UdpXml};
	use bytes::BytesMut;
	use tokio_util::codec::{Decoder, Encoder};
	...
	fn decode_c2d_s_takes_ok_arm() {
		...
		codec
			.encode(
				disc(UdpXml::C2dS(crate::baichuan::bcudp::xml::C2dS {
