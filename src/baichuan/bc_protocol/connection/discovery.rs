...
use crate::baichuan::bc_protocol::{md5_string, Md5Trunc, TcpSource};
use crate::baichuan::bcudp::codex::BcUdpCodex;
use crate::baichuan::bcudp::model::*;
use crate::baichuan::bcudp::xml::*;
...
const MTU: u32 = 1350;
/// Reolink's P2P relay pool. Entries 12-16 resolve to 127.0.0.1 on
/// Reolink's side today — presumably reserved — so they are omitted
/// rather than wasting a round of lookups on every discovery.
...
impl Discoverer {
					...
					loop {
						...
						match reader.next().await {
							Some(Ok((BcUdp::Discovery(bcudp), addr))) => {
								tracing::trace!("Got discovery {:?} for {}", bcudp, addr);
								let tid = bcudp.tid;
							...
							Some(Ok(bcudp)) => {
								...
								tracing::debug!("Got non Discovery during discovery: {:?}", bcudp);
							}
							Some(Err(e)) => {
								tracing::error!("Error on discovery socket: {:?}", e);
			...
			match subs.entry(tid) {
				...
				Entry::Occupied(mut occ) => {
					if occ.get().is_closed() {
						...
						// tracing::error!("Failed to subscribe in discovery to {:?}", tid);
	...
	async fn client_initiated_direct(
		...
		let (camera_address, camera_id, nc, pl) = self
			.retry_send(msg, addr, |bc, addr| match bc {
				UdpDiscovery {
					...
					tracing::debug!("D2cCr handshake: nc={:?} pl={:?}", nc, pl);
				...
				n => {
					tracing::debug!("Got unexpected reply: {:?}", n);
					None
				}
			})
			.await?;
		...
		tracing::debug!("Returning direct connect: {:?}", result);
		Ok(result)
	}
	...
	async fn register_address(
		...
		tracing::debug!("Registering {:?} to reolink", local_addr);
...
impl Drop for Discoverer {
	fn drop(&mut self) {
		...
		tracing::trace!("Dropped Discoverer");
	}
}
...
impl Discovery {
	...
	pub(crate) async fn get_registration(&self, uid: &str) -> Result<RegisterResult> {
		...
		for (host, err) in &dns_errors {
			tracing::warn!("Discovery: DNS lookup failed for {host}:9999 — {err}");
		}
