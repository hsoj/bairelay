...
use crate::baichuan::bc::codex::BcCodex;
use crate::baichuan::bc::model::*;
use crate::baichuan::bc_protocol::errors::BcUdpDropReceiverKind;
use crate::baichuan::bcudp::codex::BcUdpCodex;
use crate::baichuan::bcudp::{model::*, xml::*};
...
impl Drop for UdpPayloadSource {
	fn drop(&mut self) {
		tracing::trace!("Drop UdpPayloadSource");
		...
		tokio::task::spawn(async move {
			...
			tracing::trace!("Dropped UdpPayloadSource");
		});
	}
}
...
/// it from `crate::baichuan::fuzz_api`. Without the feature the parent module
...
impl UdpFlowState {
	...
	pub fn handle_data(&mut self, data: UdpData) -> bool {
		...
		if packet_id == u32::MAX {
			tracing::debug!(
				"BcUdp Data packet_id == u32::MAX; dropping (would overflow ack range)"
			);
			return false;
		}
	...
	pub fn handle_ack(&mut self, ack: UdpAck) {
		...
		self.ack_latency.feed_ack();
		tracing::trace!("sent: {}", self.sent.len());
	}
                        ...
                        break tokio::select!{
                            ...
                            packet = socket_in_rx.next() => {
                                ...
                                match tokio::time::timeout(tokio::time::Duration::from_millis(250), inner.send((packet, thread_camera_addr))).await {
                                    ...
                                    Err(_) => {
                                        tracing::trace!("Socket Error, attempting reconnect over a new one");
	...
	async fn run(&mut self) -> Result<()> {
		...
		tokio::select! {
			...
			v = self.socket_out.next() => {
					...
					match item {
						BcUdp::Discovery(UdpDiscovery{
							...
							if cid != self.flow.client_id {
								tracing::info!("Camera sent different client ID in HB");
							}
							if did != self.flow.camera_id {
								tracing::info!("Camera sent different device ID in HB");
							}
						},
				...
				tracing::trace!("Got packet");
				Ok(())
			},
		}?;
		tracing::trace!("Send");
		for payload in self.flow.drain_contiguous() {
			tracing::trace!("  + {}", self.flow.packets_want());
			self.thread_stream.feed(Ok(payload)).await?;
		}
		tracing::trace!("received: {}", self.flow.received_len());
		tracing::trace!("Flush");
...
impl Drop for UdpPayloadInner {
	fn drop(&mut self) {
		tracing::trace!("Drop UdpPayloadInner");
		self.cancel.cancel();
		...
		tokio::task::spawn(async move {
			while set.join_next().await.is_some() {}
			tracing::trace!("Dropped UdpPayloadInner");
		});
	}
}
				...
				v = async {
					loop {
						if payload_inner.thread_stream.is_closed() {
							tracing::trace!("payload_inner.thread_stream.is_closed");
							payload_inner.thread_sink.close();
							return Err(Error::BcUdpPayloadDroppedInner);
						}
						tracing::trace!("Calling inner");
						let res = payload_inner.run().await;
						tracing::trace!("Called inner: {:?}", res);
						match res {
							Ok(()) => {}
							Err(e) => {
								tracing::trace!("UDP Error. Connection will Drop: {:?}", e);
								// Pass error up
								let _ = payload_inner
									.thread_stream
...
mod tests {
	...
	//! `tokio::time::timeout`.
	use super::*;
	use crate::baichuan::bcudp::xml::{C2dHb, ClientList, UdpXml};
	use bytes::BytesMut;
