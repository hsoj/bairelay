...
use neolink_core::bc_protocol::{BcCamera, CameraDriver, StreamKind, VideoStream};
...
pub(crate) fn finish_stream_capture(
	drain_result: Result<neolink_core::bcmedia::model::BcMediaIframe>,
	output_path: Option<&Path>,
) -> Result<Outcome> {
...
pub(crate) async fn drain_first_iframe(
	...
	window: Duration,
) -> Result<neolink_core::bcmedia::model::BcMediaIframe> {
...
pub(crate) fn iframe_format_label(i: &neolink_core::bcmedia::model::BcMediaIframe) -> &'static str {
	match i.video_type {
		VideoType::H264 => "h264",
		VideoType::H265 => "h265",
	}
...
pub(crate) fn classify_stream_step(
	inner: Option<
		std::result::Result<std::result::Result<BcMedia, neolink_core::Error>, neolink_core::Error>,
	>,
) -> StreamStep {
	match inner {
...
mod tests {
	...
	fn iframe(vt: VideoType) -> neolink_core::bcmedia::model::BcMediaIframe {
		neolink_core::bcmedia::model::BcMediaIframe {
			video_type: vt,
			microseconds: 0,
			time: None,
			data: vec![1, 2, 3],
	...
	fn classify_stream_step_other_media_continues() {
		...
		// fields.
		use neolink_core::bcmedia::model::BcMediaPframe;
	...
	fn classify_stream_step_inner_err_is_read_error() {
		let step = classify_stream_step(Some(Ok(Err(neolink_core::Error::Other("parse")))));
		assert!(matches!(step, StreamStep::ReadError(_)));
	}

	#[test]
	fn classify_stream_step_outer_err_is_receive_error() {
		let step = classify_stream_step(Some(Err(neolink_core::Error::DroppedSubscriber)));
	...
	async fn run_rejects_json_without_path_before_touching_camera() {
		use neolink_core::bc_protocol::connection::mock::MockConnection;
	...
	async fn run_snap_path_via_mock_connection() {
		...
		use neolink_core::bc_protocol::connection::mock::{reply_200_xml, MockConnection};
		...
		let injector_task = tokio::spawn(async move {
			...
			let push = neolink_core::bc::model::Bc {
				meta: neolink_core::bc::model::BcMeta {
					msg_id: MSG_ID_SNAP,
					channel_id: 0,
					msg_num: 42,
					stream_type: 0,
				...
				body: neolink_core::bc::model::BcBody::ModernMsg(
					neolink_core::bc::model::ModernMsg {
						extension: Some(Extension {
							binary_data: Some(1),
							..Default::default()
						}),
						payload: Some(neolink_core::bc::model::BcPayloads::Binary(vec![
							0xDE, 0xAD, 0xBE,
						])),
					},
				),
	...
	async fn drain_skips_multiple_non_iframes_then_succeeds() {
		use neolink_core::bcmedia::model::{BcMediaAac, BcMediaPframe};
		let mut stream = MockVideoStream::new(vec![
